...
This directory contains Claude Code [hooks](https://code.claude.com/docs/en/hooks) — three advisory `PreToolUse` / `PostToolUse` handlers that surface known platform timing windows around the plugin's MCP tool calls.
...
| `advise-app-upload-poll` | PostToolUse | `confirmAppUpload` | Injects an advisory into agent context recommending the agent poll `getApp(appId)` until the async parser finishes (see closed issue [#34](https://github.com/kobiton/automate/issues/34)). |
