...
`listHostingMachines` is a new read-only tool for organization admins that lists the organization's host machines with their virtualUSB configuration (`enabled`, `network_routing`, `ip_address_configured`, `provisioning_status`) and a `virtual_usb_status` of `state` (`DISABLED`, `NO_NETWORK_ROUTE`, `ROUTING_PROVISIONING`, `ROUTING_PROVISIONING_FAILED` or `CONFIGURED`), `message` and `next_step`. The response carries a top-level `sku_active`, plus a `sku_note` when the organization lacks the virtualUSB add-on. An optional `udid` narrows the list to the host machine of one private device; when that device or its host is not visible to the caller, `machines` is empty and `reason` says why. `host_name` is null for Kobiton-hosted machines, IP addresses are never returned, and non-admins get the server's permission error unchanged. The `debug-device-over-vusb` skill calls it for a not-ready device when the user is an admin (or asks why), relays the status, and stops. The tool count goes from 39 to 40.

### New tool: configureHostingMachineVirtualUsb

`configureHostingMachineVirtualUsb` is a new tool for organization admins that enables virtualUSB on one host machine (`machineId`, the `id` from `listHostingMachines`) and sets its `networkRouting`: `KOBITON` for Kobiton-managed routing, or `SELF_MANAGED` with the machine's `ipAddress`. It cannot turn virtualUSB off and will not switch a machine from Kobiton-managed to self-managed routing, because both end active connections — do those in Portal → Device Management. A request that matches the current configuration changes nothing (`changed: false`). It returns `changed` and the `machine` in the `listHostingMachines` shape, plus `sku_active` / `sku_note`; Kobiton-managed routing starts in `ROUTING_PROVISIONING`. IP addresses are never returned. The `debug-device-over-vusb` skill offers it to an admin whose device's host is `DISABLED` or `NO_NETWORK_ROUTE` (or failed provisioning), after confirming the exact change. The tool count goes from 40 to 41.

## 1.12.0 - 2026-08-26

### Fixed: session-type enumeration now matches what the API returns

`AGENTS.md`, `CLAUDE.md`, and the README glossary listed session types as `AUTOMATION` / `CLI` / `MANUAL` / `MIXED` — a set written from expectation rather than observation; `AUTOMATION` is not a value the API emits and native instrumentation runs were missing entirely (issue #122). The enumeration is now the set `getSession` and `listSessions` actually return, verified live: `AUTO` (Appium, script- or agent-driven), `UIAUTOMATOR` / `XCUITEST` (native instrumentation via `startNativeSession` or the CLI's `test run`), `CLI`, `MANUAL`, `MIXED`, plus the rarer `SCRIPTLESS` and `GAMEDRIVER`. The docs also explain the separate `automation_type` field (`NORMAL` | `REVISIT`) and tell agents to treat an unfamiliar type as a valid session rather than an error.

### Added: CLI capabilities the bundled docs were missing

Closing the remaining gaps from issue #121 in `run-interactive-session`:

- **`test run` instrumentation** — a new "Instrumentation runs" section and command-table row document `kobiton test run --app <APP> --runner <TEST_RUNNER> <uiautomator|xcuitest>` with worked Android and iOS examples (`--follow` / `--stream`, `--test` / `--plan`, device targeting). Both surfaces that share the "test run" name now cross-reference each other: the CLI command runs your own compiled test bundle; the `createTestRun` MCP tool (README row and `create-test-run` skill) re-executes a recorded test case.
- **`device forward --mode mux|demux`** — the connection-mapping option is documented, along with the fact that transient network failures are retried for up to 5 minutes (a forward that looks stuck for a few minutes is retrying).
- **`session list`** — the new paged session listing (window, state, type, platform, keyword filters; `--all`) joins the command table alongside `session list-active` and `session show`.

### Changed: `session create --hide` everywhere, and a newer pinned CLI
