...
`listHostingMachines` is a new read-only tool for organization admins that lists the organization's host machines with their virtualUSB configuration (`enabled`, `network_routing`, `ip_address_configured`, `provisioning_status`) and a `virtual_usb_status` of `state` (`DISABLED`, `NO_NETWORK_ROUTE`, `ROUTING_PROVISIONING`, `ROUTING_PROVISIONING_FAILED` or `CONFIGURED`), `message` and `next_step`. The response carries a top-level `sku_active`, plus a `sku_note` when the organization lacks the virtualUSB add-on. An optional `udid` narrows the list to the host machine of one private device; when that device or its host is not visible to the caller, `machines` is empty and `reason` says why. `host_name` is null for Kobiton-hosted machines, IP addresses are never returned, and non-admins get an error asking them to contact an organization admin. The `debug-device-over-vusb` skill calls it for a not-ready device when the user is an admin (or asks why), relays the status, and stops. The tool count goes from 39 to 40.
