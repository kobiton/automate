...
`listDevices` gains an optional boolean `virtualUsb` (no default). With `virtualUsb: true` the result is private devices only, each carrying `virtual_usb_ready`; ready devices only unless `udid` is given, in which case the named device is returned flagged with a `virtual_usb_reason` when it is not ready (or a top-level `virtual_usb_reason` when the UDID is unknown, public, or outside the caller's access). `applied_filters` is always present on this path — including `virtualUsb: true` and a non-empty list. `deviceGroup` must be `PRIVATE` or omitted; `CLOUD` and `ALL` are rejected. When `udid` names a device that is not ready, that device also carries `virtual_usb_diagnosis` (`cause`, `message`, `next_step`): an organization admin gets the specific host-side cause plus a `host` summary and `org.sku_active`, other users get `ADMIN_REQUIRED` with an ask-an-admin message, and the diagnosis never fails the call; the `debug-device-over-vusb` skill relays it and stops. Callers that never pass `virtualUsb` see exactly the previous behaviour. The tool count stays at 39.
