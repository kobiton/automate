...
- **The named or picked device is `virtual_usb_ready: false`** → relay `virtual_usb_diagnosis.message` and `virtual_usb_diagnosis.next_step` when the diagnosis is present (for an organization admin it also carries `host`; quote `host.host_name` when present so the admin can find the machine in Portal → Device Management; it is null for Kobiton-hosted machines), otherwise relay `virtual_usb_reason` ("host machine has virtualUSB disabled or its routing is not reachable yet; check the machine in Portal → Device Management"), then **STOP**. Never try `vusb connect` on a not-ready device.
