...
| to debug their app on a real device attached to their own machine — see it in `adb` / Xcode, install a local build, read logcat ("virtualUSB", "vusb", "plug the Kobiton device into my machine") | `debug-device-over-vusb` |
...
| File | Tools |
|---|---|
| `tools/devices.yaml` | `listDevices`, `listHostingMachines`, `configureHostingMachineVirtualUsb`, `getDeviceStatus`, `reserveDevice`, `terminateReservation` |
...
The virtualUSB preflight (`skills/debug-device-over-vusb/scripts/vusb-preflight.sh`) is deliberately **not** hooked: it downloads a ~30 MB client, so it runs only from Step 1 of the `debug-device-over-vusb` skill (or by hand). Users who never invoke that skill never download it, and `/automate:setup` does not run it either.
