...
| `monitor-test-run` | **yes** — runs the bundled `scripts/poll-test-run.js` | **yes** — the poller reads it directly and has no MCP fallback | Node 18+ | **yes** — Claude Code's `Monitor`; other hosts substitute a streamed shell / watch / loop (see the host table in the skill) | no |
...
| `run-automation-suite` | **yes** — reads the user's own Appium script directory and runs it | via the runtime it invokes | Node 18+, Appium 2.x, plus the script's own language runtime (npm / python / java / dotnet / ruby) | no | no |
