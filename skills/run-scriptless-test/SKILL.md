...
- `listTestCases` or `listTestSuites` returns empty: confirm the user is in the correct organization and suggest adjusting search keywords or platform filters.
- No matching devices available: suggest using a device bundle instead of individual devices, or broadening platform/version criteria.
- `createTestRun` fails: check that the test selection and device selection are valid — e.g., test cases must match the platform of the selected devices.
- Test run stuck (no progress after several polls): poll `getTestRun` with a reasonable timeout (e.g., 15 minutes). If still running with no progress, offer to call `terminateTestRun` and retry with different devices.
