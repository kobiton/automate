...
- **Devices**: list (incl. `virtualUsb: true` to find private devices ready for virtualUSB), get status, reserve, terminate reservation
...
| to debug their app on a real device attached to their own machine — see it in `adb` / Xcode, install a local build, read logcat | `debug-virtual-usb-session` |
...
| Wants the device on *their own* machine: "real device", "plug in", "see it in adb / Xcode", "virtualUSB", "vusb", install a local build and read logcat | `debug-virtual-usb-session` |
...
| "debug on a real phone / plug the Kobiton device in / see it in adb or Xcode / vusb" | attach a real device to the user's machine over virtualUSB and debug locally | `debug-virtual-usb-session` |
...
- **`debug-virtual-usb-session`** needs a local filesystem, `~/.kobiton/.credentials`, local `adb` /
